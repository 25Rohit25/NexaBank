    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {accounts.map((account) => <Card key={account.accountId} className="shadow-sm ring-border">
          ...
          <CardContent><p className="font-heading text-3xl font-semibold">{formatMoney(Number(account.balance), account.currency)}</p><Button render={<a href={`/accounts/${account.accountId}`} />} variant="ghost" className="mt-5 px-0">Account details <ChevronRight /></Button></CardContent>
        </Card>)}
      </div>
    </div>
  );
